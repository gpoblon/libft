...
static int		ft_check_leftover(char **leftover, char **line, int i)
{
	if (*leftover)
	{
		while ((*leftover)[i] && (*leftover)[i] != '\n')
			i++;
		*line = ft_joinfree(*line, ft_subfree(*leftover, 0, i, NO_FREE),
																	FREE_ALL);
		if ((size_t)i < ft_strlen(*leftover))
		{
			*leftover = ft_subfree(*leftover, i + 1,
...
static int		ft_read_fd(char **leftover, char **line, char *buff, int fd)
{
	...
	while ((reader = read(fd, buff, BUFF_SIZE)))
	{
		...
		if (buff[i] == '\n')
		{
			*leftover = ft_subfree(buff, i + 1, reader - (i + 1), NO_FREE);
			*line = ft_joinfree(*line, ft_subfree(buff, 0, i, 0), FREE_ALL);
			return (1);
		}
		else
